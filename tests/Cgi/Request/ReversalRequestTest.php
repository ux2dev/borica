...
test('getTransactionType returns Reversal', function () {
    ...
    expect($request->getTransactionType())->toBe(TransactionType::Reversal);
});
...
test('toArray includes all mandatory fields including RRN and INT_REF', function () {
    ...
    expect($data['ORDER'])->toBe('000005');
    expect($data['DESC'])->toBe('Reversal');
...
test('optional fields appear in toArray when provided', function () {
    ...
    expect($data['AD.CUST_BOR_ORDER_ID'])->toBe('CUSTORDER5');
