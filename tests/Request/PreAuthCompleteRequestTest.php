...
test('getTransactionType returns PreAuthComplete', function () {
    $request = new PreAuthCompleteRequest(
        ...
        description: 'Pre-auth complete',
        rrn: '123456789012',
        intRef: '1234567890ABCDEF',
    );

    expect($request->getTransactionType())->toBe(TransactionType::PreAuthComplete);
});
...
test('toArray includes all mandatory fields including RRN and INT_REF', function () {
    ...
    expect($data['ORDER'])->toBe('000003');
    expect($data['DESC'])->toBe('Pre-auth complete');
...
test('optional fields appear in toArray when provided', function () {
    ...
    expect($data['AD.CUST_BOR_ORDER_ID'])->toBe('CUSTORDER3');
